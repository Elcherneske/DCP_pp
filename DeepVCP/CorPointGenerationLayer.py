...
''' 
    @param  source keypoints DFE: (B x N x 1 x 32)
            transformed keypoints DFE: (B x N x C x 32)
            candidates: B x N x C x 3
    @return target  (B x N x 3)
'''
class CorPointGenerationLayer(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, src_dfe_feat, tgt_dfe_feat, candidates, r, s):
        ...
        x = self.conv2(x)
        x = self.conv3(x)
        # output of conv3D: BN x 1 x grid_size x grid_size x grid_size
        ...
        assert not torch.any(torch.isnan(candidates))
