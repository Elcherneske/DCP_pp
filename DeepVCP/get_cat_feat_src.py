...
class Get_Cat_Feat_Src(nn.Module):
    ...
    def forward(self, src_keypts, src_keypts_grouped_pts, src_keyfeats):
        ''' 
        Input:
            src_keypts: Extracted keypts in src point cloud with normals (B x K_topk x 3)
            src_keypts_grouped_pts: 
                Grouped src_keypts with sample_and_group (B x K_topk x nsample x 3)
            src_keyfeats: 
        ...
        src_dist = pdist(torch.flatten(src_keypts_k, start_dim = 0, end_dim = 2), torch.flatten(src_keypts_grouped_pts[:, :, :, :3], start_dim = 0, end_dim = 2))
