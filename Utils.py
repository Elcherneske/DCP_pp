...
def writePoints(src, tar, src_key, tar_key, K_matrix, R_gt, t_gt, R_pred, t_pred, dir, loss):
    """
    :param src: [B, N, 4]
    :param tar: [B, N, 4]
    :param R_gt: [B, 3, 3]
    ...
    src_key = src_key.squeeze(0).to('cpu').detach().numpy()
    tar_key = tar_key.squeeze(0).to('cpu').detach().numpy()
    ...
    K_matrix = K_matrix.squeeze(0).to('cpu').detach().numpy()
    loss = loss.item()
    output_file = dir
