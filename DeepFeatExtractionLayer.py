...
if __name__ == '__main__':
    root = "../brainDataset/"
    dataset = ADNIDataset(root=root,augment=True)
    model = DeepFeatExtractionLayer(use_normal=True)
    loader = DataLoader(dataset=dataset, batch_size=1, shuffle=False)
    for n_batch, (src, target, R_gt, t_gt ) in enumerate(loader):
        # data是一个包含输入数据和对应标签的元组
        print(src.shape)
        result = model(src)
